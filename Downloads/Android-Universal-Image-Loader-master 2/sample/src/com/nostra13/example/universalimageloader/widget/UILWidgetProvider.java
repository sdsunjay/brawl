...
public class UILWidgetProvider extends AppWidgetProvider {
	...
	static void updateAppWidget(Context context, final AppWidgetManager appWidgetManager, final int appWidgetId) {
		...
		ImageSize minImageSize = new ImageSize(100, 100); // 70 - approximate size of ImageView in widget
		ImageLoader.getInstance().loadImage(Constants.dresses[0], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
		...
		ImageLoader.getInstance().loadImage(Constants.dresses[1], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
			public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
